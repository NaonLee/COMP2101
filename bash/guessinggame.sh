...
while [ $userguess != $secretnum ]: do # loop around until they get it right
  read -p "Give me a number from 1 to 10: " userguess # ask for a guess
  if [ -z $userguess ]; then echo "It is a blank!"; userguess=0
  elif [$userguess -gt 10 -o $userguess -lt 1]; then echo "The number should be between 1 and 10"
  else
