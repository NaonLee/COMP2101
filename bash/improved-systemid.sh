...
ens33_ipv4_address=$(ip a s ens33| awk -F '[/ ]+' '/inet /{print $3}')
...
ens33_network_address=$(ip route list dev ens33 scope link| cut -d ' ' -f 1)
...
Router   IP   :$default_router_address
...
for i in "${array_test[@]}"; do echo "-----$i report-----"; echo $i; done
