...
# This script demonstrates how to trap signals and handle them using function

# Task: Add traps for the INT and QUIT signals. If the script receives an INT signal,
#       reset the count to the maximum and tell the user they are not allowed to internet the count.
...
### Variables
programName="$(basename $0)"  #used by error_function.sh
sleepTime=1 # delay used by sleeptime
numberOfSleeps=10 # how many sleeps to wait for before quiting for inactivity
...
function error-message {
  echo "${programName}: ${1:_Unkown Error - a moose bit my sister once...}" >$2
}
...
function usage {
    cat <<EOF
    Usage: ${programName} [-h|--help] [-w|--waittime waittme] [-n|--waitcount waitcount]
